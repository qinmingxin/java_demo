package com.qmx.springboot.demo.common;
...
public class JWTUtils {
	 ...
	 *            jwt签发者
	 * @param subject
	 *            jwt所面向的用户。payload中记录的public claims。当前环境中就是用户的登录名。
	 * @param ttlMillis
	 *            有效期,单位毫秒
	...
	public static String createJWT(String id, String iss, String subject, long ttlMillis) {
		...
		JwtBuilder builder = Jwts.builder().setId(id) // 设置身份标志。就是一个客户端的唯一标记。 如：可以使用用户的主键，客户端的IP，服务器生成的随机数据。
				.setIssuer(iss).setSubject(subject).setIssuedAt(now) // token生成的时间。
	 ...
	 * @return
	 */
	public static ResultBean<Claims> validateJWT(String jwtStr) {
		ResultBean<Claims> checkResult = new ResultBean<>();
		Claims claims = null;
		try {
	 ...
	 * 
	 * @param jwt
	 *            就是服务器为客户端生成的签名数据，就是token。
	 * @return
	 * @throws Exception
	 */
	public static Claims parseJWT(String jwt)  {
		SecretKey secretKey = generalKey();
		return Jwts.parser().setSigningKey(secretKey).parseClaimsJws(jwt).getBody(); // getBody获取的就是token中记录的payload数据。就是payload中保存的所有的claims。
	}
	...
	public static String generalSubject(Object subObj) {
		return JSON.toJSONString(subObj);
	}
